...
export function fileContains(relativePath, substringOrPattern) {
    const desc = typeof substringOrPattern === 'string'
    ...
    return {
        ...
        run(ctx) {
            ...
            const ok = typeof substringOrPattern === 'string'
 ...
 */
export function cliHelpShowsCommands(cliAppDir, expectedCommands) {
    return {
        type: 'async',
 ...
 */
export function devStarts(timeoutMs = 5000, subdir) {
    ...
    return {
        ...
        async run(ctx) {
            ...
            const exitedFirst = await Promise.race([
                proc.exited.then(exit => ({ type: 'exited', exit })),
                sleep(timeoutMs).then(() => ({ type: 'timeout' })),
            ]);
            if (exitedFirst.type === 'exited') {
                const stderr = await new Response(proc.stderr).text();
                return fail(`Dev server exited before timeout (exit ${exitedFirst.exit}): ${stderr.slice(0, 200)}`);
            }
            proc.kill();
