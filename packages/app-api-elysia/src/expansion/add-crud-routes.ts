/**
 * scaffold api-elysia add-crud-routes — Add CRUD routes + handlers for an entity.
 */
...
import { formatEntityName, renderTemplate, toCamelCase, toPascalCase } from "@workspace/core-utils";
...
export async function executeAddCrudRoutes(
  ...
  const handlersPluginPath = resolve(pluginsDir, `handlers-${routeName}.ts`);
  ...
  const handlersPluginTemplate = readFileSync(
    join(EXPANSION_STUBS, "handlers-plugin.ts.stub"),
    "utf-8",
  );
  const context = {
  ...
  await writeFile(handlersPluginPath, renderTemplate(handlersPluginTemplate, context));

  patchRegistryForRoute(appDir, routeCamel, routeName);
  return true;
}

function patchRegistryForRoute(appDir: string, routeCamel: string, routeName: string): void {
