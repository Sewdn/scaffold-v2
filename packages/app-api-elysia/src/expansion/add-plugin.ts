/**
 * scaffold api-elysia add-plugin — Add a plugin to a scaffolded Elysia app.
 */
...
function patchIndexForPlugin(appDir: string, pluginCamel: string, pluginName: string): void {
  ...
  if (markerIdx !== -1) {
    ...
    content = content.slice(0, insertAt) + useLine + content.slice(insertAt);
  } else {
    const useCorsIdx = content.indexOf(".use(cors())");
    if (useCorsIdx !== -1) {
      const lineEnd = content.indexOf("\n", useCorsIdx);
      const insertAt = lineEnd !== -1 ? lineEnd + 1 : useCorsIdx + ".use(cors())".length;
      content =
        content.slice(0, insertAt) + `  ${SCAFFOLD_MARKER_PLUGINS}\n` + useLine + content.slice(insertAt);
    }
  }
