/**
 * scaffold api-hono add-middleware — Add middleware to a scaffolded Hono app.
 */
...
function patchIndexForMiddleware(
  ...
  if (markerIdx !== -1) {
    ...
    const appNewIdx = content.indexOf("const app = new Hono()");
    if (appNewIdx !== -1) {
      const lineEnd = content.indexOf("\n", appNewIdx);
      const insertAt = lineEnd !== -1 ? lineEnd + 1 : appNewIdx + "const app = new Hono()".length;
      content =
